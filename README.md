...
- Metabase Dashboard: визуализация результатов Inference Script для Руководителя и менеджеров отдела развития клиента.
- Real-time инференс: Переход от батчевого обновления к стримингу (Kafka) для триггеров в реальном времени (например, через 5 минут после закрытия приложения).
