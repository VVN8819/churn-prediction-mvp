# d_ml_model/dd_random_forest.py
"""
d_ml_model/dd_random_forest.py
Обучение, оценка и сохранение модели Random Forest для предсказания оттока.
"""
...
def train_and_evaluate():
    ...
    # 4. Расчет метрик
    print("\nМетрики модели Random Forest (X_test):")
    ...
    cm_path = PLOTS_DIR / "04_confusion_matrix_rf.png"
    plt.savefig(cm_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n Confusion Matrix сохранен: {cm_path}")
    
    # 6. Анализ важности признаков (коэффициенты модели)
    print("\nВажность признаков (коэффициенты Random Forest)")
    ...
    fi_path = PLOTS_DIR / "05_feature_importance_rf.png"
    plt.savefig(fi_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n График важности признаков сохранен: {fi_path}")
    
    # 7. Сохранение артефактов модели
    print("\nСохранение артефактов модели Random Forest")
        
    # Сохраняем модель
    model_path = MODELS_DIR / "random_forest_model.joblib"
    joblib.dump(model_rf, model_path, compress=3)
    print(f"- Модель сохранена: {model_path}")
    
    # Сохраняем важность признаков в CSV
    fi_csv_path = MODELS_DIR / "rf_feature_importance.csv"
